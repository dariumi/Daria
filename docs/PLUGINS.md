...
# Изменить профиль
self.api.set_user_profile("favorite_game", "quiz")
...
```python
# Добавить обмен в память вручную
self.api.add_to_conversation("Вопрос", "Ответ Даши")

# Генерация напрямую через LLM
result = self.api.generate([
    {"role": "user", "content": "Придумай идею рисунка"}
])
print(result.get("content"))

# Генерация с контекстом последних сообщений
result = self.api.generate_with_context("Что мы обсуждали?", include_history=True)
```
